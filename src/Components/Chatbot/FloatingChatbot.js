  ...
  class ActionProvider {
    ...
    faq(messageText) {
      ...
      if (lower.includes('Purchase') || lower.includes('project')) {
        ...
        response = '🧾 To view your receipts, go to the "Purchased" section in your profile.';
      } else if (lower.includes('login') || lower.includes('account')) {
        response = '🔐 Click on the "Login" button in the top right corner to sign in or create an account.';
      } else if (lower.includes('contact') || lower.includes('support')) {
        response = '📧 You can reach us via the Contact page or email us at support@tecject.in.';
      } else if (lower.includes('project') && lower.includes('types')) {
        response = '📂 We support Mini, Main, Enterprise, and Professional projects.';
      } else if (lower.includes('payment') || lower.includes('pay')) {
        response = '💳 Payments are accepted online. A receipt will be emailed after payment.';
      }  else if (lower.includes('ppt') || lower.includes('pdf') || lower.includes('documentation')) {
        response = '📑 You will receive a PPT and final report for your main project.';
      } else if (lower.includes('trustable') || lower.includes('trust') || lower.includes('loyal')) {
        response = '✅ Tecject is a reliable and trusted platform. Your projects are in good hands.';
      } else if(lower.includes('contact') || lower.includes('connect')){
        response= 'you can contact via clicking the logo at the bottom or dial 9345202170'
        ...
        response = 'To explore on project click on the domain picture in main project page via project option at top'
