...
const handleProjectsMenuOpen = (event) => {
  ...
  const roleLinks = role === 'admin' ? adminLinks : userLinks;
                ...
                <Box sx={{ width: 250 }} role="presentation" onClick={toggleDrawer(false)}>
                  <List>
                      ...
                      commonLinks.map((item) =>
                        item.subMenu ? (
                          item.subMenu.map((sub) => (
                            <ListItem button key={sub.label} onClick={() => navigate(sub.path)}>
                    ...
                    <ListItem button onClick={handleLoginLogout}>
                      <ListItemText primary={toPascalCase(isLoggedIn ? 'logout' : 'login')} />
                    </ListItem>
                  </List>
                </Box>
              </Drawer>
