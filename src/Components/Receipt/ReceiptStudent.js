...
import {
  ...
  Button, Box, Divider, IconButton, TextField
} from '@mui/material';
...
export default function StudentReceipts() {
  ...
  const handleUploadPayment = async () => {
    ...
    await updateReceiptPayment(uploadingFor.receiptId, {
      ...
      paymentProofBase64: base64 // Optional: depends on your backend support
    });
    ...
    <Box sx={{ p: 3, mt:3}}>
      ...
      <Dialog open={paymentDialog} onClose={() => setPaymentDialog(false)} maxWidth="xs" fullWidth>
        ...
        <DialogActions sx={{ justifyContent: 'center' }}>
          ...
          <Button variant="contained" onClick={handleUploadPayment} disabled={!paymentProof}>
            Upload & Confirm
          </Button>
        </DialogActions>
      </Dialog>
