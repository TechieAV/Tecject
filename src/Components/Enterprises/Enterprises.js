          ...
          <Grid container spacing={4} justifyContent="center" mt={2}>
            {methodologySteps.map((step, index) => (
              <Grid item xs={12} sm={6} md={4} key={index}>
                <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.1 }} whileHover={{ scale: 1.05 }}>
                  <Paper sx={{ p: 3, textAlign: "center" }} elevation={3}>
                    {step.icon}
                    <Typography fontWeight="bold" mt={2}>{step.title}</Typography>
                  </Paper>
                </motion.div>
              </Grid>
            ))}
          ...
          <Box mt={10} py={8} px={2} sx={{ background: "linear-gradient(to bottom, #8c8b93ff, #ffffff)", borderRadius: 2 }}>
            ...
            <Grid container spacing={4} justifyContent="center" mt={2}>
              {supportSteps.map((step, index) => (
                <Grid item xs={12} sm={6} md={4} key={index}>
                  <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.2 }}>
                    <Paper
                      sx={{
                        p: 4,
                        textAlign: "center",
                      ...
                      elevation={0}
                    >
                      {React.cloneElement(step.icon, { sx: { color: "#90caf9" } })}
                      <Typography fontWeight="bold" mt={2}>{step.title}</Typography>
                    </Paper>
                  </motion.div>
                </Grid>
